 ...
 *
 * @method ProductQuery leftJoinQuoteitem($relationAlias = null) Adds a LEFT JOIN clause to the query using the Quoteitem relation
 * @method ProductQuery rightJoinQuoteitem($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Quoteitem relation
 * @method ProductQuery innerJoinQuoteitem($relationAlias = null) Adds a INNER JOIN clause to the query using the Quoteitem relation
 *
...
abstract class BaseProductQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `idproduct`, `idproductmain`, `product_status` FROM `product` WHERE `idproduct` = :p0';
        try {
    ...
    public function filterByQuoteitem($quoteitem, $comparison = null)
    {
        if ($quoteitem instanceof Quoteitem) {
            return $this
                ->addUsingAlias(ProductPeer::IDPRODUCT, $quoteitem->getIdproduct(), $comparison);
