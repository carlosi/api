 ...
 * @method BranchQuery orderByBranchTimezone($order = Criteria::ASC) Order by the branch_timezone column
 ...
 * @method BranchQuery groupByBranchTimezone() Group by the branch_timezone column
 ...
 *
 * @method BranchQuery leftJoinBranchUserAcl($relationAlias = null) Adds a LEFT JOIN clause to the query using the BranchUserAcl relation
 * @method BranchQuery rightJoinBranchUserAcl($relationAlias = null) Adds a RIGHT JOIN clause to the query using the BranchUserAcl relation
 * @method BranchQuery innerJoinBranchUserAcl($relationAlias = null) Adds a INNER JOIN clause to the query using the BranchUserAcl relation
 *
 * @method BranchQuery leftJoinBranchdepartment($relationAlias = null) Adds a LEFT JOIN clause to the query using the Branchdepartment relation
 * @method BranchQuery rightJoinBranchdepartment($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Branchdepartment relation
 * @method BranchQuery innerJoinBranchdepartment($relationAlias = null) Adds a INNER JOIN clause to the query using the Branchdepartment relation
 *
 ...
 * @method Branch findOneByBranchTimezone(string $branch_timezone) Return the first Branch filtered by the branch_timezone column
 ...
 * @method array findByBranchTimezone(string $branch_timezone) Return Branch objects filtered by the branch_timezone column
...
abstract class BaseBranchQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `idbranch`, `idcompany`, `branch_name`, `branch_timezone`, `branch_iso_codecountry`, `branch_address`, `branch_address2`, `branch_city`, `branch_state`, `branch_zipcode` FROM `branch` WHERE `idbranch` = :p0';
        try {
    ...
    /**
     * Filter the query on the branch_timezone column
     *
     * Example usage:
     * <code>
     * $query->filterByBranchTimezone('fooValue');   // WHERE branch_timezone = 'fooValue'
     * $query->filterByBranchTimezone('%fooValue%'); // WHERE branch_timezone LIKE '%fooValue%'
     * </code>
     *
     * @param     string $branchTimezone The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return BranchQuery The current query, for fluid interface
     */
    public function filterByBranchTimezone($branchTimezone = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($branchTimezone)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $branchTimezone)) {
                $branchTimezone = str_replace('*', '%', $branchTimezone);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(BranchPeer::BRANCH_TIMEZONE, $branchTimezone, $comparison);
    }
    ...
    public function filterByBranchUserAcl($branchUserAcl, $comparison = null)
    {
        if ($branchUserAcl instanceof BranchUserAcl) {
            return $this
                ->addUsingAlias(BranchPeer::IDBRANCH, $branchUserAcl->getIdbranch(), $comparison);
    ...
    public function filterByBranchdepartment($branchdepartment, $comparison = null)
    {
        if ($branchdepartment instanceof Branchdepartment) {
            return $this
                ->addUsingAlias(BranchPeer::IDBRANCH, $branchdepartment->getIdbranch(), $comparison);
