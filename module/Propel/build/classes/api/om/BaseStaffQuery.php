...
abstract class BaseStaffQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterByStaffFirstname('fooValue');   // WHERE staff_firstname = 'fooValue'
     * $query->filterByStaffFirstname('%fooValue%'); // WHERE staff_firstname LIKE '%fooValue%'
     * </code>
     *
     * @param     string $staffFirstname The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return StaffQuery The current query, for fluid interface
     */
    public function filterByStaffFirstname($staffFirstname = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($staffFirstname)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $staffFirstname)) {
                $staffFirstname = str_replace('*', '%', $staffFirstname);
