 ...
 * @method ClientQuery orderByClientFirstname($order = Criteria::ASC) Order by the client_firstname column
 * @method ClientQuery orderByClientLastname($order = Criteria::ASC) Order by the client_lastname column
 ...
 * @method ClientQuery groupByClientFirstname() Group by the client_firstname column
 * @method ClientQuery groupByClientLastname() Group by the client_lastname column
 ...
 *
 * @method ClientQuery leftJoinClientaddressship($relationAlias = null) Adds a LEFT JOIN clause to the query using the Clientaddressship relation
 * @method ClientQuery rightJoinClientaddressship($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Clientaddressship relation
 * @method ClientQuery innerJoinClientaddressship($relationAlias = null) Adds a INNER JOIN clause to the query using the Clientaddressship relation
 *
 ...
 *
 * @method ClientQuery leftJoinTriggerprospection($relationAlias = null) Adds a LEFT JOIN clause to the query using the Triggerprospection relation
 * @method ClientQuery rightJoinTriggerprospection($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Triggerprospection relation
 * @method ClientQuery innerJoinTriggerprospection($relationAlias = null) Adds a INNER JOIN clause to the query using the Triggerprospection relation
 *
 ...
 * @method Client findOneByClientFirstname(string $client_firstname) Return the first Client filtered by the client_firstname column
 * @method Client findOneByClientLastname(string $client_lastname) Return the first Client filtered by the client_lastname column
 ...
 * @method array findByClientFirstname(string $client_firstname) Return Client objects filtered by the client_firstname column
 * @method array findByClientLastname(string $client_lastname) Return Client objects filtered by the client_lastname column
...
abstract class BaseClientQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `idclient`, `idcompany`, `client_iso_codecountry`, `client_iso_codephone`, `client_firstname`, `client_lastname`, `client_email`, `client_email2`, `client_password`, `client_cellular`, `client_phone`, `client_language`, `client_status`, `client_type` FROM `client` WHERE `idclient` = :p0';
        try {
    ...
    /**
     * Filter the query on the client_firstname column
     *
     * Example usage:
     * <code>
     * $query->filterByClientFirstname('fooValue');   // WHERE client_firstname = 'fooValue'
     * $query->filterByClientFirstname('%fooValue%'); // WHERE client_firstname LIKE '%fooValue%'
     * </code>
     *
     * @param     string $clientFirstname The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ClientQuery The current query, for fluid interface
     */
    public function filterByClientFirstname($clientFirstname = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($clientFirstname)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $clientFirstname)) {
                $clientFirstname = str_replace('*', '%', $clientFirstname);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(ClientPeer::CLIENT_FIRSTNAME, $clientFirstname, $comparison);
    }

    /**
     * Filter the query on the client_lastname column
     *
     * Example usage:
     * <code>
     * $query->filterByClientLastname('fooValue');   // WHERE client_lastname = 'fooValue'
     * $query->filterByClientLastname('%fooValue%'); // WHERE client_lastname LIKE '%fooValue%'
     * </code>
     *
     * @param     string $clientLastname The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ClientQuery The current query, for fluid interface
     */
    public function filterByClientLastname($clientLastname = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($clientLastname)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $clientLastname)) {
                $clientLastname = str_replace('*', '%', $clientLastname);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(ClientPeer::CLIENT_LASTNAME, $clientLastname, $comparison);
    }
    ...
    /**
     * Filter the query by a related Clientaddressship object
     *
     * @param   Clientaddressship|PropelObjectCollection $clientaddressship  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 ClientQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByClientaddressship($clientaddressship, $comparison = null)
    {
        if ($clientaddressship instanceof Clientaddressship) {
            return $this
                ->addUsingAlias(ClientPeer::IDCLIENT, $clientaddressship->getIdclient(), $comparison);
        } elseif ($clientaddressship instanceof PropelObjectCollection) {
            return $this
                ->useClientaddressshipQuery()
                ->filterByPrimaryKeys($clientaddressship->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByClientaddressship() only accepts arguments of type Clientaddressship or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the Clientaddressship relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return ClientQuery The current query, for fluid interface
     */
    public function joinClientaddressship($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Clientaddressship');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Clientaddressship');
        }
    ...
    /**
     * Use the Clientaddressship relation Clientaddressship object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   ClientaddressshipQuery A secondary query class using the current class as primary query
     */
    public function useClientaddressshipQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinClientaddressship($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Clientaddressship', 'ClientaddressshipQuery');
    }
     ...
     *
     * @param   Triggerprospection|PropelObjectCollection $triggerprospection  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 ClientQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
    ...
    public function filterByTriggerprospection($triggerprospection, $comparison = null)
    {
        if ($triggerprospection instanceof Triggerprospection) {
            return $this
                ->addUsingAlias(ClientPeer::IDCLIENT, $triggerprospection->getIdclient(), $comparison);
        } elseif ($triggerprospection instanceof PropelObjectCollection) {
            return $this
                ->useTriggerprospectionQuery()
                ->filterByPrimaryKeys($triggerprospection->getPrimaryKeys())
                ->endUse();
        } else {
