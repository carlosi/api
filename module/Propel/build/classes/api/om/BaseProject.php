...
abstract class BaseProject extends BaseObject implements Persistent
{
    ...
    /**
     * The value for the project_startdate field.
     * @var        string
     */
    protected $project_startdate;

    /**
     * The value for the project_enddate field.
     * @var        string
     */
    protected $project_enddate;
    ...
    /**
     * Get the [optionally formatted] temporal [project_startdate] column value.
     *
     *
     ...
     */
    public function getProjectStartdate($format = 'Y-m-d H:i:s')
    {
        if ($this->project_startdate === null) {
            return null;
        }

        if ($this->project_startdate === '0000-00-00 00:00:00') {
        ...
        try {
            $dt = new DateTime($this->project_startdate);
        } catch (Exception $x) {
            throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->project_startdate, true), $x);
        }
    ...
    /**
     * Get the [optionally formatted] temporal [project_enddate] column value.
     *
     *
     * @param string $format The date/time format string (either date()-style or strftime()-style).
     *				 If format is null, then the raw DateTime object will be returned.
     ...
     * @throws PropelException - if unable to parse/validate the date/time value.
     */
    public function getProjectEnddate($format = 'Y-m-d H:i:s')
    {
        if ($this->project_enddate === null) {
            return null;
        }

        if ($this->project_enddate === '0000-00-00 00:00:00') {
            // while technically this is not a default value of null,
            // this seems to be closest in meaning.
            return null;
        }
        ...
        try {
            $dt = new DateTime($this->project_enddate);
        } catch (Exception $x) {
            throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->project_enddate, true), $x);
        }
    ...
    /**
     * Sets the value of [project_startdate] column to a normalized version of the date/time value specified.
     *
    ...
    public function setProjectStartdate($v)
    {
        ...
        if ($this->project_startdate !== null || $dt !== null) {
            $currentDateAsString = ($this->project_startdate !== null && $tmpDt = new DateTime($this->project_startdate)) ? $tmpDt->format('Y-m-d H:i:s') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->project_startdate = $newDateAsString;
                $this->modifiedColumns[] = ProjectPeer::PROJECT_STARTDATE;
            }
    ...
    /**
     * Sets the value of [project_enddate] column to a normalized version of the date/time value specified.
     *
     * @param mixed $v string, integer (timestamp), or DateTime value.
     *               Empty strings are treated as null.
     * @return Project The current object (for fluent API support)
     */
    public function setProjectEnddate($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->project_enddate !== null || $dt !== null) {
            $currentDateAsString = ($this->project_enddate !== null && $tmpDt = new DateTime($this->project_enddate)) ? $tmpDt->format('Y-m-d H:i:s') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->project_enddate = $newDateAsString;
                $this->modifiedColumns[] = ProjectPeer::PROJECT_ENDDATE;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->project_startdate = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
            $this->project_enddate = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
            $this->resetModified();
            ...
            return $startcol + 6; // 6 = ProjectPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(ProjectPeer::PROJECT_STARTDATE)) {
            $modifiedColumns[':p' . $index++]  = '`project_startdate`';
        }
        if ($this->isColumnModified(ProjectPeer::PROJECT_ENDDATE)) {
            $modifiedColumns[':p' . $index++]  = '`project_enddate`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`project_startdate`':
                        $stmt->bindValue($identifier, $this->project_startdate, PDO::PARAM_STR);
                        break;
                    case '`project_enddate`':
                        $stmt->bindValue($identifier, $this->project_enddate, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[4] => $this->getProjectStartdate(),
            $keys[5] => $this->getProjectEnddate(),
        );
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) $this->setProjectStartdate($arr[$keys[4]]);
        if (array_key_exists($keys[5], $arr)) $this->setProjectEnddate($arr[$keys[5]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProjectPeer::PROJECT_STARTDATE)) $criteria->add(ProjectPeer::PROJECT_STARTDATE, $this->project_startdate);
        if ($this->isColumnModified(ProjectPeer::PROJECT_ENDDATE)) $criteria->add(ProjectPeer::PROJECT_ENDDATE, $this->project_enddate);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setProjectStartdate($this->getProjectStartdate());
        $copyObj->setProjectEnddate($this->getProjectEnddate());
