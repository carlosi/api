...
abstract class BaseToken extends BaseObject implements Persistent
{
    ...
    /**
     * Get the [optionally formatted] temporal [expires_in] column value.
     *
     *
     * @param string $format The date/time format string (either date()-style or strftime()-style).
     *				 If format is null, then the raw DateTime object will be returned.
     ...
     * @throws PropelException - if unable to parse/validate the date/time value.
     */
    public function getExpiresIn($format = 'Y-m-d H:i:s')
    {
        if ($this->expires_in === null) {
            return null;
        }

        if ($this->expires_in === '0000-00-00 00:00:00') {
            // while technically this is not a default value of null,
            // this seems to be closest in meaning.
            return null;
        }
        ...
        try {
            $dt = new DateTime($this->expires_in);
        } catch (Exception $x) {
            throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->expires_in, true), $x);
        }
    ...
    public function setIdtoken($v)
    {
        ...
        if ($this->idtoken !== $v) {
            $this->idtoken = $v;
            $this->modifiedColumns[] = TokenPeer::IDTOKEN;
        }
    ...
    public function setIduser($v)
    {
        ...
        if ($this->iduser !== $v) {
            $this->iduser = $v;
            $this->modifiedColumns[] = TokenPeer::IDUSER;
        }
    ...
    /**
     * Sets the value of [expires_in] column to a normalized version of the date/time value specified.
     *
     * @param mixed $v string, integer (timestamp), or DateTime value.
     *               Empty strings are treated as null.
     * @return Token The current object (for fluent API support)
    ...
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->expires_in !== null || $dt !== null) {
            $currentDateAsString = ($this->expires_in !== null && $tmpDt = new DateTime($this->expires_in)) ? $tmpDt->format('Y-m-d H:i:s') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->expires_in = $newDateAsString;
                $this->modifiedColumns[] = TokenPeer::EXPIRES_IN;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->idtoken = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
            ...
            $this->expires_in = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
            $this->resetModified();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(TokenPeer::IDTOKEN)) {
            $modifiedColumns[':p' . $index++]  = '`idtoken`';
        }
        if ($this->isColumnModified(TokenPeer::IDUSER)) {
            $modifiedColumns[':p' . $index++]  = '`iduser`';
        }
        if ($this->isColumnModified(TokenPeer::TOKEN)) {
            $modifiedColumns[':p' . $index++]  = '`token`';
        }
        if ($this->isColumnModified(TokenPeer::CREATED_AT)) {
            $modifiedColumns[':p' . $index++]  = '`created_at`';
        }
        if ($this->isColumnModified(TokenPeer::EXPIRES_IN)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`created_at`':
                        ...
                        break;
                    case '`expires_in`':
                        $stmt->bindValue($identifier, $this->expires_in, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[4], $arr)) $this->setExpiresIn($arr[$keys[4]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TokenPeer::IDTOKEN)) $criteria->add(TokenPeer::IDTOKEN, $this->idtoken);
        if ($this->isColumnModified(TokenPeer::IDUSER)) $criteria->add(TokenPeer::IDUSER, $this->iduser);
        ...
        if ($this->isColumnModified(TokenPeer::EXPIRES_IN)) $criteria->add(TokenPeer::EXPIRES_IN, $this->expires_in);
    ...
    {
        $criteria = new Criteria(TokenPeer::DATABASE_NAME);
        $criteria->add(TokenPeer::IDTOKEN, $this->idtoken);
