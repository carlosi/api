...
//
//        $idcompany_array=array(1,2);
//        $client_status = array('pending','active','suspended','fraud');
//        $client_type = array('NORMAL','GENERALPUBLIC','INVENTORYMANAGER');
//        
//        for($i=0;$i<100;$i++){
//            $client = new Client();
//            $client->setIdcompany($idcompany_array[array_rand($idcompany_array)]);
//            $client->setClientIsoCodecountry(52);
//            $client->setClientIsoCodephone(52);
//            $client->setClientFullname(\Shared\DataTest::$last_names[array_rand(\Shared\DataTest::$last_names)]);
//            $client->setClientEmail(strtolower($client->getClientFullname()).'@gmail.com');
//            $client->setClientEmail2(strtolower($client->getClientFullname()).'@hotmail.com');
//            $client->setClientPassword('123456');
//            $client->setClientCellular(rand(1111111111, 9999999999));
//            $client->setClientPhone(rand(11111111, 99999999));
//            $client->setClientLanguage('ES');
//            $client->setClientType($client_type[array_rand($client_type)]);
//            $client->setClientStatus($client_status[array_rand($client_status)]);
//            $client->save();
//            
//        }
        ...
        if($userLevel!=0){
            ...
            */
            ...
            foreach ($result['data'] as $item){
                ...
                if(key($row) == 'client_iso_codecountry'){
                    if(!$row['client_iso_codecountry']){
                        unset($row['client_iso_codecountry']);
                    }
                }

                if(key($row) == 'client_iso_codephone'){
                    if(!$row['client_iso_codephone']){
                        unset($row['client_iso_codephone']);
                    }
                }
                if(key($row) == 'client_email'){
                    if(!$row['client_email']){
                        unset($row['client_email']);
                    }
                }
                if(key($row) == 'client_email2'){
                    if(!$row['client_email2']){
                        unset($row['client_email2']);
                    }
                }
                if(key($row) == 'client_password'){
                    if(!$row['client_password']){
                        unset($row['client_password']);
                    }
                }
                if(key($row) == 'client_cellular'){
                    if(!$row['client_cellular']){
                        unset($row['client_cellular']);
                    }
                }
                if(key($row) == 'client_phone'){
                    if(!$row['client_phone']){
                        unset($row['client_phone']);
                    }
                }
                if(key($row) == 'client_language'){
                    if(!$row['client_language']){
                        unset($row['client_language']);
                    }
                }
                if(key($row) == 'client_type'){
                    if(!$row['client_type']){
                        unset($row['client_type']);
                    }
                }
