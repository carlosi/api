...
//==============FORMS================
use REST\v1\Company\ACL\ClientFile\Form\ClientFileFormPostPut;
use REST\v1\Company\ACL\Client\Form\ClientFormGET;
use REST\v1\Company\ACL\ClientFile\Form\ClientFileFormGET;
//==============FILTERS==============
use REST\v1\Company\ACL\ClientFile\Filter\ClientFileFilterPostPut;
//=============PROPEL===============
use Clientfile;
use ClientQuery;
use BasePeer;
        ...
        if($userLevel!=0){
            ...
            if($clientfileForm->isValid()){
                ...
                if($result->count()==1){
                    ...
                    //Modifiamos el Header de nuestra respuesta
                    $response = $this->getResponse();
                    $response->getHeaders()->addHeaderLine('Location', WEBSITE_API.'/'.$this->table.'/'.$clientfile->getIdclientfile());
                    $response->setStatusCode(\Zend\Http\Response::STATUS_CODE_201);

                    //Le damos formato a nuestra respuesta
                    $bodyResponse = array(
                        "_links" => array(
                            'self' => WEBSITE_API.'/'.$this->table.'/'.$clientfile->getIdclientfile(),
                        ),
                    );
                    ...
                    $bodyResponse ['_embedded'] = array(
                        'client' => array(
                            '_links' => array(
                                'self' => array('href' =>  WEBSITE_API.'/'.$this->table.'/'.$clientfile->getIdclient()),
                            ),
                        ),
                    );
        ...
        if($userLevel!=0){
            ...
            if($result!=null){
                ...
                $clientfileArray ['_embedded'] = array(
                    'client' => array(
                        '_links' => array(
                            'self' => array('href' => WEBSITE_API.'/client/'.$clientfile->getIdclient()),
                        ),
                    ),
                );
    ...
    public function getList() {
        ...
        if($userLevel!=0){
            ...
            foreach ($result['data'] as $item){
                ...
                $row = array(
                    "_links" => array(
                        'self' => array('href' => WEBSITE_API.'/'.$this->table.'/'.$item['idclientfile']),
                    ),
                );
                foreach ($clientfileForm->getElements() as $key=>$value){
                ...
                if($this->getQuery()->create()->useClientQuery()->filterByIdCompany($idCompany)->endUse()->filterByIdclientfile($id)->exists()){
                    ...
                    if($clientfileForm->isValid()){
                        ...
                        if($clientfile->isModified()){
                            ...
                            $bodyResponse = array(
                                "_links" => array(
                                    'self' => WEBSITE_API.'/'. $this->table.'/'.$clientfile->getIdClient(),
                                ),
                            );
