...
//==============FORMS================
use Company\V1\REST\ACL\BranchUser\Form\BranchUserFormGET;
use Company\V1\REST\ACL\BranchUser\Form\BranchUserFormPostPut;
use Company\V1\REST\ACL\Branch\Form\BranchFormGET;
use Company\V1\REST\ACL\User\Form\UserFormGET;
//==============FILTERS==============
use Company\V1\REST\ACL\BranchUser\Filter\BranchUserFilterPostPut;
//=============PROPEL===============
use BranchUserQuery;
use BranchUser;
use BranchQuery;
        ...
        if($userLevel!=0){
            ...
            if($branchuserForm->isValid()){
                ...
                if($result->count()==1){
                    ...
                    //Modifiamos el Header de nuestra respuesta
                    $response = $this->getResponse();
                    $response->getHeaders()->addHeaderLine('Location', URL_API.'/'.$this->table.'/'.$branchuser->getIdbranchUser());
                    $response->setStatusCode(\Zend\Http\Response::STATUS_CODE_201);

                    //Le damos formato a nuestra respuesta
                    $bodyResponse = array(
                        "_links" => array(
                            'self' => URL_API.'/'.$this->table.'/'.$branchuser->getIdbranchUser(),
                        ),
                    );
                    ...
                    $bodyResponse ['_embedded'] = array(
                        'branch' => array(
                            '_links' => array(
                                'self' => array('href' =>  URL_API.'/'.$this->table.'/'.$branchuser->getIdbranch()),
                            ),
                        ),
                        'user' => array(
                            '_links' => array(
                                'self' => array('href' =>  URL_API.'/'.$this->table.'/'.$branchuser->getIduser()),
                            ),
                        ),
                    );
                    ...
                    if(isset($data['idbranch']) && $data['idbranch'] != null){
                        ...
                        if($branchuserForm->isValid()){
                            ...
                            if($branchuser->isModified()){
                                ...
                                $bodyResponse = array(
                                    "_links" => array(
                                        'self' => URL_API.'/'. $this->table.'/'.$branchuser->getIdbranch(),
                                    ),
                                );
                                ...
                                $bodyResponse ['_embedded'] = array(
                                    'branch' => array(
                                        '_links' => array(
                                            'self' => array('href' => URL_API.'/company/'.$branchuser->getIdBranch()),
                                        ),
                                    ),
                                    'user' => array(
                                        '_links' => array(
                                            'self' => array('href' => URL_API.'/company/'.$branchuser->getIduser()),
                                        ),
                                    ),
                                );
